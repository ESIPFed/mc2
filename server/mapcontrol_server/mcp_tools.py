...
async def update_style(map_id: str, asset_id: str, style: AssetStyle) -> dict[str, Any]:
    ...
    animation) to draw attention while it's being processed; "done" stops
    the pulse and marks it complete (full opacity, green stroke); "muted"
    grays it out (no longer under consideration). For custom effects use
    style.animate = [{"property": "opacity"|"circle_radius"|"stroke_width",
    "from": .., "to": .., "period": seconds}] ([] stops animation), and
    style.opacity for a flat 0..1 opacity.
